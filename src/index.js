...
bot.command('start', async (ctx) => {
    // TODO: reply with functionality
    await ctx.reply('Welcome')

    const { id, username, first_name, last_name } = ctx.from
    await createUser(id, ctx.chatId, username, `${first_name} ${last_name}`.trim())
});
...
bot.callbackQuery(brands.map(({ data }) => data),async (ctx) => {
...
bot.callbackQuery(sexes.map(({ data }) => data),async (ctx) => {
    const { label } = sexes.find(({data}) => data === ctx.match);
    ctx.session.sex = label;
    await ctx.reply('Осталось определиться с размером. Можно выбрать несколько:', { reply_markup: sizeKeyboard });
    await ctx.answerCallbackQuery();
});

bot.callbackQuery(sizes.map(({ data }) => data),async (ctx) => {
...
bot.callbackQuery('confirm',async (ctx) => {
    ...
    if (sizes.length) {
        if ([process.env.ADMIN_ID, process.env.TECH_SUPPORT_ID].includes(ctx.from.id.toString())) {
            await ctx.reply(`Данные успешно собраны.\n\nБренд: ${ctx.session.brands.join('; ')}\nРазмер: ${ctx.session.sizes.join('; ')}\nПол: ${ctx.session.sex}\n\nОтправьте команду /clear для очистки данных или введите сообщение для рассылки.`);
            await ctx.answerCallbackQuery();
            return;
        }
...
bot.command('my_subscriptions', async (ctx) => {
    const subscriptions = (await getUserSubscriptions(ctx.chatId)).map(({ sex, brand, size }, index) => {
        return `${index + 1}) ${brand} ${size} ${sex}`
    }).join('\n\n');

    await ctx.reply(subscriptions || 'У вас нет текущих подписок.\n\nЧтобы подписаться на рассылку отправьте команду /subscribe');
});

bot.command('unsubscribe', async (ctx) => {
    const subscriptions = (await getUserSubscriptions(ctx.chatId)).map(({ sex, brand, size }) => {
        return `${brand} ${size} ${sex}`
    });

    const subscriptionsKeyboard = subscriptions.reduce((result, label, index) => result.text(label, `unsubscribe_${index}`).row(), new InlineKeyboard())
    subscriptionsKeyboard.text('Отписаться от всех', 'unsubscribe_all')

    await ctx.reply(
        `Выберите рассылку, от которой хотите отписаться:`,
...
bot.command('post', async (ctx) => {
    ...
    if (![process.env.ADMIN_ID, process.env.TECH_SUPPORT_ID].includes(user_id.toString())) {
...
bot.command('subscriptions', async (ctx) => {
    const user_id = ctx.from.id;
    if (![process.env.ADMIN_ID, process.env.TECH_SUPPORT_ID].includes(user_id.toString())) {
        await ctx.reply('Для просмотра текущих подписок отправьте команду /my_subscriptions');
        return
    }

    const subscriptions = (await getSubscriptions()).map(({ sex, brand, size }, index) => {
        return `${index + 1}) ${brand} ${size} ${sex}`
    }).join('\n');
...
bot.command('users', async (ctx) => {
    ...
    const users = (await getUsers()).map(({ name, username }, index) => `${index + 1}) ${name} (@${username})`).join('\n');
...
bot.on('message', async (ctx) => {
    ...
    if (sizes.length && sex && brands) {
        ...
        await ctx.reply('Сообщение готово к публикации. Выберите действие:', { reply_markup: postKeyboard });
    } else {
...
bot.callbackQuery('send',async (ctx) => {
    ...
    const users = await getFilteredSubscriptionUsers(sex, brands, sizes)

    for (const user of users) {
        if (message?.photo) {
            await ctx.api.sendPhoto(user.chat_id, message.photo, { caption: message.caption })
        }
        if (message.text) {
